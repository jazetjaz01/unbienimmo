...
export function LoginForm({
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Une erreur est survenue avec Google"
      );
    ...
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <Card className="w-full max-w-md mx-auto">
        <CardHeader className="space-y-1">
          <CardTitle className="text-2xl text-center font-medium tracking-tight">
            Bienvenue !
          </CardTitle>
        ...
        <CardContent className="grid gap-4">
          ...
          >
            {isGoogleLoading ? (
              ...
              <svg className="w-4 h-4" viewBox="0 0 24 24">
                ...
                <path
                  fill="#EA4335"
                  d="M12 4.75c1.77 0 3.35.61 4.6 1.8l3.42-3.42C17.95 1.19 15.24 0 12 0 7.31 0 3.18 2.12 1.21 5.64l4.11 3.22c.94-2.84 3.57-4.95 6.68-4.95z"
                />
              </svg>
            )}
          ...
          <form onSubmit={handleEmailLogin} className="space-y-4">
            {/* ... champs email et password inchangés ... */}
            ...
            <Button
              type="submit"
              className="w-full mt-2 bg-slate-800 hover:bg-black text-white"
              disabled={isLoading || isGoogleLoading}
            >
              {isLoading ? (
          ...
          <div className="text-center text-sm text-slate-600 mt-2">
            ...
            <Link href="/auth/sign-up" className="text-cyan-700 font-medium hover:underline">
              Créer un compte
            </Link>
          </div>
        </CardContent>
